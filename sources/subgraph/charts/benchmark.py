...
from sources.subgraph.utils import date_to_timestamp
from sources.subgraph.constants import WETH_ADDRESS
from sources.subgraph.charts.config import BASE_POOLS_CONFIG, WETH_USDC_POOL
...
class Benchmark:
    ...
    async def chart(self, v2=False):
        ...
        df_lp = pd.DataFrame(data["v3"]["lpDayData"], dtype=np.float64).set_index(
            "date"
        )
